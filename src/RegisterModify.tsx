...
const RegisterModify: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    if (inputQuantity)
      inputQuantity.value = String(basketItems[itemIndex]?.quantity);
    inputQuantity?.focus();
  ...
  const save = (e: React.FormEvent) => {
    ...
    if (basketItems[itemIndex]) {
      const existingIndex = basketItems.findIndex(
        (item) => item.product.code === basketItems[itemIndex].product.code
      );
      if (existingIndex >= 0) {
        ...
        if (toNumber(discountText) > 0 || toNumber(rateText) > 0) {
          ...
          if (toNumber(discountText) > 0) {
            ...
            discountPrice = -Math.floor(
              (Number(basketItems[itemIndex].product.sellingPrice) *
                toNumber(rateText)) /
                100.0
            );
          }
          ...
          if (
            basketItems[existingIndex + 1] &&
            !basketItems[existingIndex + 1].product.code
          ) {
  ...
  return (
    <Modal
      open={open && !!basketItems[itemIndex]}
      size="none"
      onClose={onClose}
      className="w-1/2"
    >
        ...
        <Table border="row" className="table-fixed w-full">
          <Table.Body>
            ...
            <Table.Row>
              ...
              <Table.Cell>
                {basketItems[itemIndex]?.product.sellingPrice?.toLocaleString()}
              </Table.Cell>
            </Table.Row>
            <Table.Row>
              ...
              <Table.Cell>
                <Form onSubmit={save} className="space-y-2">
                  <Form.Text
                    ...
                    onBlur={() =>
                      setQuantityText(toNumber(quantityText).toString())
                    }
              ...
              <Table.Cell>
                <Form onSubmit={save} className="space-y-2">
                  <Form.Text
                    ...
                    onBlur={() =>
                      setDiscountText(toNumber(discountText).toString())
                    }
                    className="text-right w-full"
                  />
                </Form>
      ...
      <Modal.Footer className="flex justify-end space-x-2">
        <Button
          color="primary"
          disabled={toNumber(quantityText) <= 0}
          onClick={save}
        >
          OK
        </Button>
