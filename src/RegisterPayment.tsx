    ...
    runTransaction(db, async (transaction) => {
      ...
      const sale: Sale = {
        ...
        taxTotal:
          ...
          inclusiveTaxNormalTotal +
          exclusiveTaxReducedTotal +
          inclusiveTaxReducedTotal,
        discountTotal: 0,
        paymentType,
  ...
  const priceNormalTotal = ((items: BasketItem[]) => {
    return (
      items
        ...
        .reduce(
          (result, item) =>
            result + Number(item.product.sellingPrice) * item.quantity,
          0
        ) * registerSign
    );
  })(basketItems);
      ...
      Math.floor(
        (items
          .filter(
            (item) =>
              item.outputReceipt &&
              item.product.sellingTaxClass === "exclusive" &&
              item.product.sellingTax === 10
          )
          .reduce(
            (result, item) =>
              result + Number(item.product.sellingPrice) * item.quantity,
      ...
      Math.floor(
        (items
          .filter(
            (item) =>
              item.outputReceipt &&
              item.product.sellingTaxClass === "inclusive" &&
              item.product.sellingTax === 10
          )
          .reduce(
            (result, item) =>
          ...
          <Table.Body>
                    ...
                    onKeyPress={(e) => {
                      if (e.key === "Enter" && handlePrint) {
                        ...
                        if (toNumber(cashText) >= salesTotal) {
                          if (
                            basketItems.some((item) => item.outputReceipt) &&
                            handlePrint
                          ) {
                            handlePrint();
                          } else {
            ...
            <Table.Row className={registerMode === "Return" ? "hidden" : ""}>
              ...
              <Table.Cell className="text-right pr-4">
                ¥
                {toNumber(cashText) < salesTotal
                  ? "0"
                  : (toNumber(cashText) - salesTotal).toLocaleString()}
              </Table.Cell>
            </Table.Row>
          </Table.Body>
        </Table>
        ...
        <div className="hidden">
          <div ref={componentRef} className="p-10">
            <p className="text-right text-sm mt-2">
              {currentTimestamp.toDate().toLocaleDateString()}{" "}
            ...
            <p className="text-center text-xl font-bold m-2">
              {registerMode === "Return" ? "返品" : "領収書"}
            </p>
            <Table
              border="cell"
              className="table-fixed w-full text-sm shadow-none"
            >
              <Table.Head>
                <Table.Row>
                  ...
                  ?.map((basketItem, index) => (
                    <Table.Row key={index}>
                      ...
                      <Table.Cell className="text-right">
                        ¥
                        {(
                          Number(basketItem.product.sellingPrice) *
                          basketItem.quantity
                        )?.toLocaleString()}
                      </Table.Cell>
                    </Table.Row>
            ...
            <Flex className="mt-4">
              ...
              <Table
                border="none"
                size="sm"
                className="table-fixed w-2/3 shadow-none"
              >
                <Table.Head>
                  <Table.Row>
                ...
                <Table.Body>
                  <Table.Row>
                    ...
                    <Table.Cell className="text-right text-xl pr-4">
                      ¥{salesExceptHidden.toLocaleString()}
                    </Table.Cell>
                  ...
                  <Table.Row>
                    ...
                    <Table.Cell className="text-right pr-4">
                      ¥{priceTaxFreeTotal.toLocaleString()}
                    </Table.Cell>
                    <Table.Cell></Table.Cell>
                  </Table.Row>
                  <Table.Row>
                  ...
                  {registerMode === "Return" ||
                  basketItems.some((item) => !item.outputReceipt) ? null : (
                    <Table.Row>
                      <Table.Cell type="th">お預かり</Table.Cell>
                      <Table.Cell className="text-right pr-4">
                        ¥{toNumber(cashText).toLocaleString()}
                      </Table.Cell>
                      <Table.Cell></Table.Cell>
                    </Table.Row>
                  )}
